...
import {ShoppingCartIcon, ChevronUpIcon, ChevronDownIcon} from '@heroicons/react/24/outline'
...
function Success({products}: Props) {
  ...
  return (
    <div>
        <Head>
            <title>Thank you! -William</title>
        </Head>
        <header className='mx-auto max-w-xl'>
        <Link href="/">
            <div className='relative ml-4 h-16 w-8 cursor-pointer transition lg:hidden'>
                <Image alt='t' src={'/images/apple-logo.png'} layout='fill' objectFit='contain'/>
            </div>
        </Link>
        </header>
        ...
        <div className='my-8 ml-4 flex space-x-4 lg:ml-14 xl:ml-0'>
            <div className='flex h-11 w-11 items-cennter justify-center rounded-full border-2 border-black'>
                <CheckIcon className='h-8 w-8'/>
            </div>
            <div>
                <p className='text-sm text-gray-600'>Order Number {session_id?.slice(-5)}</p>
                <h4 className='text-lg'>Thank you 
                    {session ? session.user?.name?.split(" ")[0] : ' Guest'}
                </h4>
            </div>
        </div>
        <div className='mx-4 divide-y divied-gray-300 rounded-md border border-gray-300 p-4 lg:ml-14'>
            <div className='space-y-2 pb-3'>
                <p >Your order is confirmed</p>
                <p className='text-sm text-gray-600'>We've accepted your orderm and we're getting it ready. Come back to this page for updates on your shipment status.</p>
        ...
        <div className='mx-4 flex flex-col items-center justify-between text-sm lg:ml-14 lg:flex-row'>
            ...
            {mounted && (
                <section className='overflow-y-screoll border-y border-l border-gray-300 bg-[#fafafa] lg:order-2 lg:col-span-4 lg:h-screen lg:border-y-0'>
                        ...
                        <div className='mx-auto max-w-xl divide-y border-gray-300 px-4 py-4 lg:mx-0 lg:max-w-lg lg:px-10 lg:py-16 '>
                            <div className='space-y-4 pb-4'>
                                {products.map((product) => (
                                    <div className='flex items-center space-x-4 font-medium text-sm' key={product.id}>
                                        <div className='relative flex h-16 w-16 items-center justify-center rounded-md border border-gray-300 bg-[#f1f1f1] text-xs text-white'>
                                            <div className='relative h-7 w-7 animate-bounce rounded-md'>
                                                <Image layout='fill' objectFit='contain' alt="t" src='/images/apple-logo.png'/>
                                            </div>
                                        ...
                                        <p>
                                            <Currency currency={product.currency} quantity={product.price.unit_amount / 100}/> 
                                        </p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className='flex justify-between pt-4'>
                                <p>Total</p>
                                <p className='flex items-center gap-x-2 text-xs text-[gray]'>USD                                 <span className='text-xl font-medium text-black'>
                                    <Currency quantity={subtotal + 20}/>
                                </span></p>
                            </div>
                        </div>
